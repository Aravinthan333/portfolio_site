...
export function AdminPageHeader({
  ...
  return (
    <div className="flex flex-col gap-4 border-b border-border pb-6 sm:flex-row sm:items-start sm:justify-between">
      <div>
        {backHref && (
          <a href={backHref} className="mb-2 inline-block text-sm text-accent hover:underline">
            ← {backLabel}
          </a>
        )}
...
export function AdminHeroButton({
  ...
  const className =
    variant === "primary"
      ? "inline-flex items-center gap-2 rounded-full bg-accent px-4 py-2 text-sm font-semibold text-background hover:bg-accent-hover"
